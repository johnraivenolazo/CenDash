...
if (isset($_GET['res_del'])) {
    $deleteVendor = $_GET['res_del'];
    $stmt = $db->prepare("DELETE FROM vendor WHERE rs_id = :id");
    $stmt->execute([':id' => $deleteVendor]);
                            ...
                            <div class="card card-outline-primary">
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="example23"
                                        ...
                                        <tbody>
                                            <?php
                                            $stmt = $db->query("SELECT * FROM vendor ORDER BY rs_id DESC");
                                            $vendors = $stmt->fetchAll();

                                            if (count($vendors) == 0) {
                                                echo '<td colspan="11"><center>No Vendors</center></td>';
                                            } else {
                                                foreach ($vendors as $rows) {
                                                    $catStmt = $db->prepare("SELECT * FROM vendor_group WHERE c_id = :c_id");
                                                    $catStmt->execute([':c_id' => $rows['c_id']]);
                                                    $row = $catStmt->fetch();
                                                    echo ' 
                                                    <tr>
                                                    ...
                                                    <td>' . htmlspecialchars($rows['o_hr']) . '</td>
                                                    <td>' . htmlspecialchars($rows['c_hr']) . '</td>
                                                    <td>' . htmlspecialchars($rows['o_days']) . '</td>	
                                                    <td><div class="col-md-3 col-lg-8 m-b-10">
                                                    <center><img src="/assets/vendorassets/' . htmlspecialchars($rows['image']) . '" class="img-responsive radius"  style="width:50px;height:50px;"/></center>
                                                    </div></td>
                                                    <td>' . htmlspecialchars($rows['date']) . '</td>
                                                    <td><a href="?res_del=' . $rows['rs_id'] . '" class="btn btn-danger btn-flat btn-addon btn-xs m-b-10"><i class="fa fa-trash" style="font-size:16px"></i></a> 
